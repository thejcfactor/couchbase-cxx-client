...
class cluster_config_tracker_impl
  ...
  , public impl::background_bootstrap_listener
{
    public:
        ...
        void
        create_sessions(utils::movable_function<void(std::error_code, topology::configuration cfg)>&& handler)
        {
                ...
                if (!ec) {
                    if (self->origin_.options().network == "auto") {
                        ...
                        if (self->origin_.options().network == "default") {
                            CB_LOG_DEBUG(R"({} detected network is "{}")", new_session.log_prefix(), self->origin_.options().network);
                        } else {
                            CB_LOG_INFO(R"({} detected network is "{}")", new_session.log_prefix(), self->origin_.options().network);
                        }
                    }
                    if (self->origin_.options().network != "default") {
                    ...
                    CB_LOG_WARNING(R"({} failed to bootstrap cluster session ec={}")", new_session.log_prefix(), ec.message());
                }
                //asio::post(asio::bind_executor(self->ctx_, [h = std::move(h), ec, cfg = std::move(cfg)]() mutable { h(ec); }));
                h(ec, cfg);
            });
        }

        void downgrade_to_single_session()
        {
            if (closed_) {
                return;
            }
            ...
            CB_LOG_DEBUG(R"({} Had num sesssions: "{}")", log_prefix_, sessions_.size());
            ...
            std::vector<io::mcbp_session> old_sessions;
            {
                ...
                for (auto it = sessions_.begin(); it != sessions_.end();) {
                    if(first) {
                        ...
                        old_sessions.emplace_back(std::move(*it));
            ...
            CB_LOG_DEBUG(R"({} Now have num sesssions: "{}")", log_prefix_, sessions_.size());
        }
        ...
        {
            std::set<std::shared_ptr<impl::bootstrap_notification_subscriber>> subscribers;
            {
        ...
        void notify_bootstrap_success() override
        {
            ...
            for (const auto& subscriber : subscribers) {
                subscriber->notify_bootstrap_success();
            }
        }

        void register_bootstrap_notification_subscriber(
            std::shared_ptr<impl::bootstrap_notification_subscriber> subscriber) override
        {
            std::scoped_lock lock(bootstrap_notification_subscribers_mutex_);
    ...
    private:
        ...
        void update_config_sessions(topology::configuration config)
        {
            ...
            for (auto it = new_sessions.begin(); it != new_sessions.end(); ++it) {
                ...
                ctx_, [session = std::move(*it)]() mutable { return session.stop(retry_reason::do_not_retry); }));
            }
        }
        ...
        void restart_sessions()
        {
            ...
            for (std::size_t index = 0; index < config_->nodes.size(); ++index) {
                ...
                auto ptr = std::find_if(sessions_.begin(), sessions_.end(), [&hostname, &port](const auto& session) {
                    return session.bootstrap_hostname() == hostname && session.bootstrap_port_number() == port;
                });
        ...
        std::set<std::shared_ptr<impl::bootstrap_notification_subscriber>> bootstrap_notification_subscribers_{};
...
cluster_config_tracker::cluster_config_tracker(std::string client_id,
                        couchbase::core::origin origin,
                        asio::io_context& ctx,
                        asio::ssl::context& tls,
                        std::shared_ptr<impl::bootstrap_state_listener> state_listener)
...
void
cluster_config_tracker::create_sessions(utils::movable_function<void(std::error_code, topology::configuration cfg)>&& handler)
{
    return impl_->create_sessions(std::move(handler));
}

void
cluster_config_tracker::downgrade_to_single_session()
{
    return impl_->downgrade_to_single_session();
}
...
void cluster_config_tracker::notify_bootstrap_success()
{
    return impl_->notify_bootstrap_success();
}

void
cluster_config_tracker::register_bootstrap_notification_subscriber(
    std::shared_ptr<impl::bootstrap_notification_subscriber> subscriber)
{
