...
#include "core/impl/background_bootstrap_listener.hxx"
    ...
    if (origin_.exhausted()) {
      if(background_bootstrap_listener_) {
        if (last_bootstrap_error_.has_value()) {
          ...
          background_bootstrap_listener_->notify_bootstrap_error({errc::network::no_endpoints_left,
...
private:
  void invoke_bootstrap_handler(std::error_code ec)
  {
    ...
    if (!bootstrapped_ && bootstrap_callback_) {
      ...
      if (config_ && state_listener_) {
        ...
        // TODO: maybe unnecessary and background_bootstrap_listener can notify on the bootstrap callback?
        if(background_bootstrap_listener_) {
          background_bootstrap_listener_->notify_bootstrap_success();
        }
      }
      auto h = std::move(bootstrap_callback_);
      h(ec, config_.value_or(topology::configuration{}));
    }
    // TODO: keep the session or allow for new one (created by owner of session(s))?
    // If connecting in the background, we retry on failure until the cluster is exposed.
    if (ec && background_bootstrap_listener_) {
      return initiate_bootstrap();
    }
    else if (ec) {
    ...
    if (it != endpoints_.end()) {
      ...
      auto error_msg =
        fmt::format("no more endpoints left to connect to \"{}:{}\", will try another address",
                    bootstrap_hostname_,
                    bootstrap_port_);
      last_bootstrap_error_ = { errc::network::no_endpoints_left,
                                std::move(error_msg),
                                bootstrap_hostname_,
                                bootstrap_port_ };
  ...
  std::shared_ptr<impl::background_bootstrap_listener> background_bootstrap_listener_{ nullptr};
};
...
void
mcbp_session::add_background_bootstrap_listener(std::shared_ptr<impl::background_bootstrap_listener> listener)
{
