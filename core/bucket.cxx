...
public:
  ...
  auto direct_dispatch(std::shared_ptr<mcbp::queue_request> req) -> std::error_code
  {
    ...
    if (!routed.session || !routed.session->has_config()) {
      if (!routed.connect_pending) {
        // No bootstrap was started for this node, and only a bootstrap
        // completion or a bucket close drains the deferred queue. Deferring
        // here would leave the request waiting for its deadline, so retry
        // instead: that re-routes against the current configuration.
    ...
    -> std::error_code
  {
    ...
    if (!routed.session || !routed.session->has_config()) {
      if (!routed.connect_pending) {
        ...
        // deferred request would wait for its deadline instead of being sent.
        if (backoff_and_retry(req, retry_reason::node_not_available)) {
          return {};
        }
  ...
  struct routed_session {
    ...
    /**
     * False when no session exists for the node and no bootstrap was started
     * for it, so nothing will drain the deferred queue. The caller must retry
     * rather than defer; see connect_session().
     *
     * The default is false. route_request() returns a default-constructed
     * value when the configuration names no server for the vbucket, and a
     * request deferred on that has nothing to wait for.
     */
  ...
  /**
   * A command deferred for this index is only ever dispatched or completed by
   * one of two events: a bootstrap finishing, or the bucket closing.
   *
   * @return true when one of them is coming. Either a bootstrap is now pending
   * for this index, or the bucket is closed and defer_command() completes the
   * command with a cancellation. False when neither is, in which case the
   * caller must retry: a command deferred now would wait for its deadline.
   */
        ...
        if (auto found_kv_node_index = ptr->first; found_kv_node_index != kv_node_index) {
          if (auto current = sessions_.find(kv_node_index); current == sessions_.end()) {
            ...
            CB_LOG_WARNING(
              ...
              ptr->second.id(),
              kv_node_index,
              logger::system_data(current->second.bootstrap_address()),
              current->second.id());
    ...
    for (auto ptr = sessions_.cbegin(); ptr != sessions_.cend();) {
      if (ptr->second.id() == id) {
        CB_LOG_DEBUG(R"({} removed session id="{}", address="{}", bootstrap_address="{}")",
                     log_prefix_,
                     ptr->second.id(),
                     logger::system_data(ptr->second.remote_address()),
                     logger::system_data(ptr->second.bootstrap_address()));
        ...
        for (auto it = sessions_.begin(); it != sessions_.end(); ++it) {
          if (it->second.bootstrap_hostname() == hostname &&
            ...
            CB_LOG_DEBUG(R"({} rev={}, preserve session="{}", address="{}", index={}->{})",
                         ...
                         config.rev_str(),
                         it->second.id(),
                         logger::system_data(it->second.bootstrap_address()),
                         it->first,
                         next_index);
  ...
  [[nodiscard]] auto find_or_connect_session_by_index(std::size_t index) -> routed_session
  {
    if (auto session = find_session_by_index(index); session) {
      // A session exists, so it is safe to defer behind it. That holds even
      // if it is still bootstrapping: the completion that finishes it is what
